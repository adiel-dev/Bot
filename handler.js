        ...
        for (let name in global.plugins) {
            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                            m.reply(`*[ ⚠️ 𝚁𝙴𝙿𝙾𝚁𝚃𝙴 𝙳𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾 𝙲𝙾𝙽 𝙵𝙰𝙻𝙻𝙾𝚂 ⚠️ ]*\n\n*—◉ 𝙿𝙻𝚄𝙶𝙸𝙽:* ${name}\n*—◉ 𝚄𝚂𝚄𝙰𝚁𝙸𝙾:* ${m.sender}\n*—◉ 𝙲𝙾𝙼𝙰𝙽𝙳𝙾:* ${m.text}\n\n*—◉ 𝙴𝚁𝚁𝙾𝚁:*\n\`\`\`${format(e)}\`\`\`\n\n*⚔️ Reportelo al Creador del Bot Para Darle una Solucion, Use el Comando #reporte ⚔️*`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*⚔️ Sus Diamantes se han Agotado, Puede Comprar mas Usando el Comando ⚔️ ${usedPrefix}buy <cantidad>*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `*⚔️ Se requiere el Nivel ⚔️ ${plugin.level} Para usar este Comando, Tu Nivel es ⚔️ ${_user.level}*`, m)
...
//this.sendButton(id, text, groupMetadata.subject, apii.data, [[(action == 'add' ? '🔥 BIENVENIDO 🔥' : '⚔️ 𝙰𝙳𝙸𝙾𝚂 ⚔️'), (action == 'add' ? '#welcomegc' : '#byegc')], ['🌀 𝙼𝙴𝙽𝚄 𝙳𝙴 𝙲𝙾𝙼𝙰𝙽𝙳𝙾𝚂 🌀', `#menu`]], null, {mentions: this.parseMention(text)})
...
export async function callUpdate(callUpdate) {
    ...
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Netfree Mx 👑;;;\nFN:Netfree Mx 👑\nORG:Netfree Mx 👑\nTITLE:\nitem1.TEL;waid=17059900962:+1 705 990 0962\nitem1.X-ABLabel:Netfree Mx 👑\nX-WA-BIZ-DESCRIPTION:🍒 Contacta a Este Numero Para Cosas Importantes 🍒\nX-WA-BIZ-NAME:Netfree Mx 👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: 'Netfree Mx 👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    ...
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] AVISO - ALERTA', body: 'THE BOT - NETFREE', thumbnail: imagen1, sourceUrl: 'https://github.com/netfree-dev/bot' }}}}, aa)
